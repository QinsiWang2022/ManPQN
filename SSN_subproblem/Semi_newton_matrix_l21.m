function [Z,inner_iter,Lam,r_l,stop_flag] = Semi_newton_matrix_l21(n,r,X,t, B,mut,inner_tol,prox,inner_max_iter,Lam0,Dn,pDn)
...
[Z,delta,Inact_set] = prox(X_Lam_prod,mut);
ZX = Z'*X;
R_Lam= ZX+ZX'-2*eye(r);
RE=pDn*R_Lam(:);
%  s = s - r_s/gd;
...
while  r_l^2 > inner_tol
    ...
    Lam_new = Lam + new_d ;
    [Z, delta,Inact_set]=prox(X_Lam_new_prod,mut);
    ZX=Z'*X;
    ...
    if r_l_new<=0.99*sigma
        ...
        X_V_prod=B+2*t*X*V;
        [Z,delta,Inact_set]=prox(X_V_prod,mut);
