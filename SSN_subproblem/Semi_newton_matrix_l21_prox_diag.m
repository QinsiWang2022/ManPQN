function [Z,inner_iter,Lam,r_l,stop_flag] = Semi_newton_matrix_l21_prox_diag(n,r,X,diag_invD,t,B,mut,inner_tol,prox,inner_max_iter,Lam0,Dn,pDn)
%find the zero of X'*Z(Lam)+ Z(Lam)'*X=2*I, where z(Lam) is the proximal
%point of B+2*t*diag_invD.*(X*Lam)      %prox_number=0;
XtX = eye(r);
Xt  = X';
% global  Dn pDn
numb=0; stop_flag=0;
...
[Z,delta,Inact_set] = prox(X_Lam_prod,mut,diag_invD);
ZX = Z'*X;
R_Lam= ZX+ZX'-2*eye(r);
RE=pDn*R_Lam(:);
%  s = s - r_s/gd;
...
while  r_l^2 > inner_tol
    ...
    for i=1:r
        for j = 1:r
            for k = 1:n
                g((i*r-r+1:i*r),(j*r-r+1:j*r)) =g((i*r-r+1:i*r),(j*r-r+1:j*r)) + ...
                    X(k,i)*(diag_invD(k).*X(k,j))*delta{k};
            end
        end
    end
    G = 4*t*pDn*(g*Dn);
    ...
    Lam_new = Lam + new_d ;
    [Z, delta,Inact_set]=prox(X_Lam_new_prod,mut,diag_invD);
    ZX=Z'*X;
    ...
    if r_l_new<=0.99*sigma
        ...
        X_V_prod=B+2*t*X*V;
        [Z,delta,Inact_set]=prox(X_V_prod,mut,diag_invD);
