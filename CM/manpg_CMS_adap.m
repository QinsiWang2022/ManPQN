function [X_manpg, F_manpg,F,sparsity,time_manpg,iter_adap,flag_succ,num_linesearch,mean_ssn] =manpg_CMS_adap(H,option,d_l,V)
%min -Tr(X'*H*X)+ mu*norm(X,1) s.t. X'*X=Ir. where H is the discretized Schro?dinger operator
%%
%parameters
tic;
...
%     fprintf(1,print_format, iter-1,min(F), time_manpg,sparsity, inner_flag,mean_ssn  ,sqrt(normDsquared)/t,num_linesearch);
end
